...
| `AUTH_DB_PASSWORD`  | authorization-server                | — (fail-fast)                                | Sem default no config-server. **Docker secret, não env:** o compose monta o secret `POSTGRES_PASSWORD` no auth-server com `target: AUTH_DB_PASSWORD`, resolvido pelo configtree. |
...
| `OAUTH_END_SESSION_URI`    | gateway               | `http://localhost:5173/connect/logout` (compose) · `http://localhost:8082/connect/logout` (`@Value`, dev manual) | Endpoint de logout OIDC do IdP (browser). Lido em `gateway/.../SecurityConfig.java`. Aponta para a **origem do SPA**, não para o auth-server — ver a nota abaixo. |
...
| `SESSION_MAX_LIFETIME`            | authorization-server  | `8h`              | Teto de vida da sessão do IdP (`security.session.max-lifetime`, [ADR-025](adr/ADR-025-revalidacao-estado-emissao.md)), contado do **instante de autenticação** — não da criação da sessão, que no BFF é anterior ao login (CSRF + saved request) e faria um login legítimo nascer vencido. Distinto de `SESSION_TIMEOUT`, que é **inatividade** e cada autorização renova. **Granularidade real ≈ 60 min** (vida do refresh token): quem renova por `refresh_token` não passa pelo `/oauth2/authorize`. |
| `SESSION_REVALIDATION_ENABLED`    | authorization-server  | `true`            | **Reversão operacional, não configuração suportada** (ADR-025). Desliga a re-derivação do titular na emissão do authorization code. Existe porque o raio de um defeito no filtro é *"ninguém consegue logar"* e o rollback precisa ser restart (config-server), não rebuild — mesmo precedente de `TOKEN_REVOCATION_ENABLED`. Em `false` o sistema **volta ao comportamento vulnerável** do incidente de 2026-08-07 (sessão do IdP refabricando credencial limpa a partir de estado obsoleto): use como botão de emergência e reverta. |
| `REVOCATION_JWK_SET_URI`          | gateway               | `${AUTH_ISSUER_URI}/oauth2/jwks` | JWKS do `RevocationTokenReader` (ADR-025), que lê `userID`/`iat` do access token da sessão verificando **assinatura** e ignorando `exp` — sem isso a checagem de revogação da borda era pulada com token expirado. Endereço **interno** (rede Docker), como `AUTH_ISSUER_URI`. **Valor errado não quebra nada visível:** o decoder falha sempre, o filtro cai no fail-open e a correção fica inerte com build verde; o sinal é o WARN `\| revogação \| falha na checagem de borda (fail-open) \| causa: …`. |

> ² O `RegisteredClient` no authorization-server já permite a URI de `:5173`, para o callback aterrissar no SPA.

> **`OAUTH_END_SESSION_URI` tem de ser same-origin com o SPA.** O logout do ADR-018 é um **POST
> top-level via `<form>`** (`login-interface/src/api/authClient.ts`), e a CSP que o nginx emite no
> documento do SPA termina em `form-action 'self'`. O Chrome aplica essa diretiva **também ao
> redirect** da submissão: um 302 para outra origem é recusado dentro do browser, sem request. O
> sintoma é traiçoeiro — o 1º clique encerra a sessão e apaga o cookie `XSRF-TOKEN` (o servidor
> processou), mas a página não navega; do 2º em diante o `_csrf` vai vazio e o gateway devolve
> **403 Access Denied**. Por isso o default dev aponta para `:5173/connect/logout`, que o
> `location /connect/` do nginx encaminha ao gateway (rota `connect-logout`, `GatewayRouter`), e o
> deploy usa `${PUBLIC_ORIGIN}/connect/logout`: a cadeia é a mesma nos dois ambientes. Em **dev
> manual** a variável não é injetada e vale o default do `@Value` (`:8082`) — lá não há nginx nem
> CSP; o proxy do Vite encaminha `/connect` para o caso de o gateway vir do Docker.
...
| `API_BASE_URL`                         | user-service           | `http://localhost:8081`  | Base do link de confirmação (`app.verification.base-url`) — **único** consumidor desde que o `servers[]` do OpenAPI virou relativo (ver [§ Swagger/OpenAPI](#swagger--openapi)). Deve apontar à borda que o titular alcança pelo browser, nunca ao user-service interno. |
...
> **O `servers[]` do doc do user-service não tem variável — é relativo (`/`), por decisão.** Uma URL
> absoluta ali casa com uma só das origens que servem a página do Swagger (nginx do SPA em `:5173` e
> em deploy, proxy do Vite em dev manual, gateway em `:8081`); nas demais o "Try it out" vira
> cross-origin e o browser o barra no `connect-src 'self'` da CSP que o nginx emite em
> `/swagger-ui`, com um "Failed to fetch" sem diagnóstico. Relativo, o Swagger-UI resolve contra a
> origem da própria página: a chamada é sempre same-origin, a CSP passa e o cookie `SESSION` viaja
> junto para o `tokenRelay()`. `API_BASE_URL` **não** volta a esse papel — hoje serve só ao link de
> verificação de e-mail. Fixado por `OpenAPIConfigTest` no user-service.
...
>
> **`API_BASE_URL` saiu do bloco `gateway`** dos dois compose (2026-09-22), pelo mesmo critério: o gateway nunca a leu — ele importa só o `config/gateway.yml`, que não a referencia, e não há `@Value` correspondente em `gateway/src`. A env **permanece no user-service**, único consumidor (`config/user-service.yml` → `app.verification.base-url`); o que foi removido é a injeção inútil no gateway. **Não reintroduzir lá.**
>
> **Deploy via Cloudflare Tunnel (`docker-compose.deploy.yml`):** todas as envs de borda derivam de `${PUBLIC_ORIGIN}`. `AUTH_URL`/`AUTH_TOKEN` **permanecem** e alimentam só o securityScheme do doc OpenAPI (`OpenAPIConfig`) — quem os usa é o browser na página do Swagger, não o back-channel do BFF. O overlay os define nos **dois** serviços que os leem (gateway **e** user-service). `API_BASE_URL` também é derivada de `${PUBLIC_ORIGIN}` ali, mas **só no user-service** e **pelo link de verificação de e-mail**, não pelo Swagger: o `servers[]` é relativo e não depende de env alguma. O botão **Authorize** deixou de completar o fluxo (ADR-020) — sob o BFF o "Try it out" autentica pelo cookie `SESSION` e o `tokenRelay()` da rota, sem token no browser.
>
...
> opostos sob hostname único: o `config-lb` só balanceia as réplicas do config-server na rede
> interna (resolvidas por DNS, ADR-024), enquanto o `interface` é o **funil de 100% do tráfego público** — o túnel
