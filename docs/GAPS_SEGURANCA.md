...
> **Resolvidos (removidos desta lista):** G2 (C16), G3 (C17), G4 (C11), G6 (C18), G7 (C12), G8 (C8) e G10 (C19), além do JWT em `localStorage` no front-end (BFF). Os IDs são preservados (a numeração não é reusada); detalhes no histórico git. Os controles ativos correspondentes estão em [Controles de segurança já implementados](#controles-de-segurança-já-implementados).
