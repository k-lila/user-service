...
- [~] **TLS/HTTPS** (ref **G1**) — **borda TLS de dev entregue** (curativo); TLS de produção segue para a infra de deploy. **Implementado:** overlay opt-in `docker-compose.tls.yml` sobe um reverse-proxy **nginx** (`infra/tls-proxy`) que **termina TLS** com cert do **mkcert** e fala HTTPS com o browser, mantendo o tráfego interno em HTTP. Dois hostnames `*.localhost` (sem `/etc/hosts`): `app.localhost` (SPA+BFF → `interface:80`) e `auth.localhost` (front-channel OAuth2 → `authorization-server:8082`). Reusa o split front/back-channel já existente — o back-channel (`AUTH_ISSUER_URI`) segue HTTP interno, **sem truststore Java**. Ajustes: `OAuth2ClientConfig` ganha `redirectUri`/`postLogoutRedirectUri` para `https://app.localhost`; `login-interface/nginx.conf` faz pass-through do `X-Forwarded-Proto`; gateway liga `Secure` nos cookies via `app.cookie.secure` (`APP_COOKIE_SECURE`); auth-server liga via `server.forward-headers-strategy`; `RateLimiterConfig`/`RateLimitLogFilter` leem o IP via `getHostString()` (com `forward-headers-strategy=framework` o WebFlux consome o `X-Forwarded-For` e o `remoteAddress` vira `InetSocketAddress` *unresolved* — `getAddress().getHostAddress()` dava NPE/500 no `/users/register` e no log de 429); Swagger-UI funciona no modo TLS pela borda (`AUTH_URL`/`AUTH_TOKEN` → `https://auth.localhost` no user-service; CORS do auth-server em CSV incluindo `http://localhost:8081`). Subida: `docker compose -f docker-compose.yml -f docker-compose.tls.yml up -d --build`. Setup mkcert e verificação em [TLS_DEV.md](TLS_DEV.md). **Pendência:** cert ACME/corporativo + domínios reais no ambiente de produção. · **Prioridade: Alta · Esforço: M**
