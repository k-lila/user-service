...
- [~] **C17 — Proteger/segregar o config-server** (ref **G3**). **Feito nesta rodada:** porta `8888` (`config-lb`) não publicada em prod (via C16) + **defaults de secret removidos** dos YAMLs servidos (`AUTH_DB_USER`/`AUTH_DB_PASSWORD` em `authorization-server.yml`; `OAUTH_CLIENT_SECRET` ×2 em `gateway.yml`) → sem default, a env ausente no cliente derruba a subida (fail-fast, como C11). **Pendente:** autenticação no endpoint (Spring Security Basic/mTLS) — adiada (confia no isolamento de rede nesta rodada). · **Prioridade: Média · Esforço: M**
