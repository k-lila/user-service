...
- [docs/ARQUITETURA.md](docs/ARQUITETURA.md) — arquitetura estrutural: camadas de cada serviço, fluxos ponta a ponta, contratos entre serviços e árvore de arquivos anotada
...
O `docker-compose.yml` é **base prod-safe** e **não publica porta nenhuma** no host — nem `gateway`, nem `interface` (G10/ADR-019 as moveu para o override; a asserção (e) do smoke-test de login guarda isso). Quem publica é o `docker-compose.override.yml` (auto-carregado por `docker compose up`, portas internas para **dev**) e o `docker-compose.deploy.yml` (só Grafana/Prometheus/Zipkin em `127.0.0.1` — a borda pública chega pelo túnel, não por `ports:`). Consequência a ter em mente: `docker compose -f docker-compose.yml up` sobe a topologia inteira **inalcançável do host**, o que é o ponto (validar a topologia, servir de base ao overlay) e não um deploy utilizável sozinho. Corolário útil: como a base não publica nada, em **deploy** o `--scale` não tem conflito de porta em serviço de aplicação algum — a ressalva do `-f` explícito vale para **dev**, onde o override publica portas fixas. Todos os serviços declaram `cpus` / `mem_limit` / `mem_reservation` (chaves de nível de serviço, não `deploy.resources`) — perfis e racional em [docs/CONFIG.md § Limites de recursos](docs/CONFIG.md#limites-de-recursos-cpu--memória). Dois pontos a **não** regredir ali: os 4 apps Spring usam o perfil `x-res-jvm` (`cpus: 2.0`), **separado** do `x-res-app` dos nós Mongo — fundi-los dobraria a quota do MongoDB sem motivo; e o heap/coletor vêm de `JAVA_TOOL_OPTIONS` explícito (`-XX:+UseG1GC -XX:MaxRAMPercentage=60`) porque, sem ele, a ergonomia da JVM classifica o container como *client-class machine* e cai em `UseSerialGC` com heap de 256m.
