...
- **Testes (312):** 136 unitários (Mockito/reativos) + 64 controller (`@WebMvcTest` — 59 `UserControllerTest` + 5 `InternalUserControllerTest`) + 72 integração (user-service: Mongo+Redis; auth-server: Postgres+Redis+WireMock, fluxo OAuth2; gateway: Redis+WireMock via `WebTestClient`, roteamento/rate-limit/CSRF + BFF ponta a ponta; config-server: `MockMvc` p/ HTTP Basic) + 40 front-end (Vitest+RTL+MSW, threshold 80%).
