...
- **Prometheus** — `/actuator/prometheus`, scrape 5s; job `microservices` (discovery×2 nas portas principais `9091`/`9092`; auth-server, user-service, notification-service e gateway na **porta de management `8181`**, não na de tráfego — ver G14 fechado em `docs/SECURITY.md`) + job `config-server` (config-server-1/2:8888, `basic_auth` — o `/actuator/prometheus` do config-server fica atrás de HTTP Basic; a senha já vem do Docker secret via `password_file: /run/secrets/CONFIG_SERVER_PASSWORD`, não inline); 3 exporters de infra (sem `ports:` no base — prod-safe):
