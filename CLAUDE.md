...
- **Testes (368):** 157 unitários (Mockito/reativos, incl. `AdminServiceTest`) + 92 controller (`@WebMvcTest` — 59 `UserControllerTest` + 5 `InternalUserControllerTest` + 31 `AdminControllerTest`) + 79 integração (user-service: Mongo+Redis, incl. `AdminFlowIntegrationTest`; auth-server: Postgres+Redis+WireMock, fluxo OAuth2; gateway: Redis+WireMock via `WebTestClient`, roteamento/rate-limit/CSRF + BFF ponta a ponta + `GatewayAdminRouteIntegrationTest`; config-server: `MockMvc` p/ HTTP Basic) + 40 front-end (Vitest+RTL+MSW, threshold 80%).
